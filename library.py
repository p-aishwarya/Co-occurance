...
def terms_to_graph(terms, w):
    ...
    print("from_to",from_to,len(from_to))
    print("Terms--------",sorted(set(terms)))
    # create empty graph
    g = igraph.Graph(directed=True)
    
    # add vertices
    g.add_vertices(list(sorted(set(terms))))
    
    # add edges, direction is preserved since the graph is directed
    g.add_edges(list(from_to.keys()))
    # print("g==========",g)
    # print("*********************")
    # print("keys---",list(from_to.keys()))
    # print("values---",list(from_to.values()))
    # set edge and vertex weights
    g.es['weight'] = list(from_to.values()) # based on co-occurence within sliding window
    # print("-------------",g.es['weight'], len(g.es['weight']))
    # print("=============================================")
    # print("************",from_to.values(),type(from_to.values()))
    # g.vs['weight'] = g.strength(weights=list(from_to.values())) # weighted degree
    g.vs['weight'] = g.strength(list(from_to.values()),weights=g.es['weight'])
